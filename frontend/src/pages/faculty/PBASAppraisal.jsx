...
import { authApi, pbasApi, academicYearApi } from '../../api/services';
  ...
  async function load() {
    try {
      ...
      try {
        ...
        const list = Array.isArray(ayRes.data) ? ayRes.data : [];
        if (list.length > 0) {
      ...
      try {
        const appraisalRes = await pbasApi.getMyAppraisal(currentYear);
...
// Fetch appraisal when academicYear changes
useEffect(() => {
  ...
  async function loadAppraisal() {
    ...
    try {
      ...
      console.error("Failed to load appraisal for selected year:", err);
    }
  }
...
const handleRecall = async () => {
  ...
  setRecalling(true);
  try {
    await pbasApi.recallAppraisal(appraisalId);
    setAppraisalStatus('DRAFT');
  } catch (err) {
      ...
      <div className="relative z-10">
          ...
          <div>
            <div className="flex items-center gap-3">
              ...
              <select
                ...
                className="bg-white/10 border border-white/20 text-white text-xs font-bold rounded-lg px-2 py-1 outline-none focus:ring-2 focus:ring-white/50"
              >
        ...
        {facultyProfile && (
          <div className="flex flex-wrap gap-2 mt-3">
            ...
            <span className={`px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-wide ${appraisalStatus === 'DRAFT' ? 'bg-amber-500/20 text-amber-200' :
            ...
            {appraisalStatus === 'SUBMITTED' && (
              <button
                ...
                disabled={recalling}
                className="ml-auto px-3 py-1 rounded-xl text-xs font-bold text-white bg-rose-500/80 hover:bg-rose-500 transition-colors shadow-sm shadow-rose-900/20 disabled:opacity-50 flex items-center gap-1.5 cursor-pointer"
              >
